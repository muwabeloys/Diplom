...
if (count.count > 0) {
    console.log('Правила уже добавлены. Удалите таблицу для повторного заполнения.');
    process.exit(0);
}
...
const rules = [
    {
        ...
        description: 'Настоящее простое время. Используется для регулярных действий, фактов и привычек. He/She/It + глагол с окончанием -s.',
    ...
    },
    {
        ...
        description: 'Настоящее длительное время. Используется для действий, происходящих прямо сейчас. am/is/are + глагол-ing.',
        examples: JSON.stringify([
    ...
    },
    {
        language: 'en',
        title: 'Past Simple',
        description: 'Прошедшее простое время. Правильные глаголы +ed, неправильные — 2-я форма. Для завершённых действий в прошлом.',
        examples: JSON.stringify([
    ...
    },
    {
        ...
        description: 'A/An — неопределённый артикль (один из многих, первое упоминание). The — определённый артикль (конкретный предмет, уже известный).',
        examples: JSON.stringify([
            { en: 'I have a cat. The cat is black.', ru: 'У меня есть кот. (Этот) кот чёрный.' },
    ...
    },
    {
        ...
        description: 'Предлоги места: in (внутри), on (на поверхности), at (в точке/месте), under (под), between (между), next to (рядом).',
        examples: JSON.stringify([
    ...
    },
    {
        ...
        examples: JSON.stringify([
            ...
            { en: 'I won\'t be late.', ru: 'Я не опоздаю.' }
        ]),
        category: 'tenses',
        level: 'intermediate',
        order_index: 6
    },
    {
        ...
        description: 'Настоящее совершённое время. Have/has + 3-я форма глагола. Связь прошлого с настоящим, результат.',
        examples: JSON.stringify([
    ...
    },
    {
        language: 'en',
        title: 'Modal Verbs',
        description: 'Модальные глаголы: can (мочь/уметь), must (должен), should (следует), may (можно). Не изменяются по лицам, после них глагол без to.',
        examples: JSON.stringify([
    ...
    },
    {
        ...
        description: 'Zero Conditional: If + Present Simple, Present Simple (факты). First Conditional: If + Present Simple, will + verb (реальные условия).',
        examples: JSON.stringify([
            ...
            { en: 'If you study, you will pass.', ru: 'Если будешь учиться, сдашь.' }
        ]),
    ...
    },
    {
        ...
        examples: JSON.stringify([
            ...
            { en: 'The letter has been sent.', ru: 'Письмо отправлено.' }
        ]),
        category: 'verbs',
        level: 'advanced',
        order_index: 10
